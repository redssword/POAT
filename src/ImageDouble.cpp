...
// filtrage : moyen, gaussien, ecart-type (approches par vecteur et transposée plus rapide)
CImageDouble CImageDouble::filtrage(const std::string& methode, int N, double sigma) {
	...
	if (methode.compare("moyen") == 0) {
		...
		else if (methode.compare("ecart-type") == 0) {
			out.m_sNom = this->lireNom() + "filtrage_ecartType";

			for (unsigned int i = 0; i < this->lireHauteur(); i++) {
				for (unsigned int j = 0; j < this->lireLargeur(); j++) {
					double somme = 0;
					double moy = 0;
					double ecartType = 0;
					int nbPixels = 0;
...
double CImageDouble::moyenneLocale(const CImageDouble& img, int i1, int j1, int i2, int j2) {
	int haut = i2 - i1 + 1;
	int larg = j2 - j1 + 1;
	double A = (i1 == 0 || j1 == 0) ? 0 : img(i1 - 1, j1 - 1);
	double B = (i1 == 0) ? 0 : img(i1 - 1, j2);
	double C = img(i2, j2);
	double D = (j1 == 0) ? 0 : img(i2, j1 - 1);
	double moyenne = (C - D - B + A) / (double)(haut * larg);
	return moyenne;
}

double CImageDouble::ecartTypeLocal(const CImageDouble& integrale1, const CImageDouble& integrale2, int i1, int j1, int i2, int j2) {
	int haut = i2 - i1 + 1;
	int larg = j2 - j1 + 1;
	double A = (i1 == 0 || j1 == 0) ? 0 : integrale2(i1 - 1, j1 - 1);
	double B = (i1 == 0) ? 0 : integrale2(i1 - 1, j2);
	double C = integrale2(i2, j2);
	double D = (j1 == 0) ? 0 : integrale2(i2, j1 - 1);
	double somme = C - D - B + A;

	double moyenne = moyenneLocale(integrale1, i1, j1, i2, j2);

	double ecTypeLocal = std::sqrt((somme / (double)(haut * larg)) - (moyenne * moyenne));
	return ecTypeLocal;
}

CImageNdg CImageDouble::moyennesLocale(const CImageNdg& img, int X, int Y) {
	//Remplie this avec l'image intégrale1
	CImageDouble DoubleIntegrale1(img, "integrale1");
	*this = DoubleIntegrale1;

	CImageNdg img_moyennes(img);

	int i1, j1, i2, j2;
	int decalageX = X / 2;
	int decalageY = Y / 2;
	int hauteur = img.lireHauteur();
	int largeur = img.lireLargeur();

	for (int i = 0; i < hauteur; i++) {
		for (int j = 0; j < largeur; j++) {
			//Calcul des coordonnées
			i1 = (i - decalageX) < 0 ? 0 : (i - decalageX);
			j1 = (j - decalageY) < 0 ? 0 : (j - decalageY);
			i2 = (i + decalageX) >= hauteur ? (hauteur - 1) : i + decalageX;
			j2 = (j + decalageY) >= largeur ? (largeur - 1) : j + decalageY;

			// Stocke la valeur de la moyenne locale dans le pixel
			img_moyennes(i, j) = static_cast<unsigned char>(moyenneLocale(*this, i1, j1, i2, j2));
		}
	}
	return img_moyennes;
}

CImageNdg CImageDouble::ecartTypeLocaux(const CImageNdg& img, int X, int Y) {
	//Remplie this avec l'image intégrale1
	CImageDouble DoubleIntegrale1(img, "integrale1");
	CImageDouble DoubleIntegrale2(img, "integrale2");
	*this = DoubleIntegrale2;

	CImageNdg img_ecartTypes(img);
	...
	for (int i = 0; i < hauteur; i++) {
		for (int j = 0; j < largeur; j++) {
			...
			// Stocke la valeur de la moyenne locale dans le pixel
			img_ecartTypes(i, j) = static_cast<unsigned char>(ecartTypeLocal(DoubleIntegrale1, DoubleIntegrale2, i1, j1, i2, j2));
		}
	}
	return img_ecartTypes;
}

CImageNdg  CImageDouble::maximumLocaux()
{
	CImageDouble NORME;
